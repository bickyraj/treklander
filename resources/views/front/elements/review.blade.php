<div class="review">
    <h3 class="mb-4 text-2xl text-gray-600 font-display">{{ $review->title }}</h3>
    <div class="mb-8 prose">
        <p>{{ $review->review }}</p>
    </div>
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            <img src="{{ $review->thumbImageUrl }}" alt="{{ $review->review_name }}" class="object-cover w-16 h-16 rounded-full border-accent" loading="lazy">
            <div>
                <div class="font-bold text-gray-600">{{ ucfirst($review->review_name) }}</div>
                <div class="text-sm">{{ $review->review_country }}</div>
                <div class="flex">
                    @for ($i = 0; $i < 5; $i++)
