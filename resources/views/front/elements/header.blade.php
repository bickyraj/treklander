...
{{-- Header --}}
<header class="@if (!request()->routeIs('front.trips.show')) fixed @endif flex items-center w-full transition h-20 header shadow" x-data="{ mobilenavOpen: false, searchboxOpen: false }">
    <div class="container relative flex items-end justify-between w-full">
        <!-- Logo -->
        <a class="flex-shrink-0" href="{{ route('home') }}">
            <img src="{{ asset('assets/front/img/logo-h.webp') }}" class="block w-auto py-2 h-14 brand" alt="{{ config('app.name') }}" width="640" height="438">
        </a><!-- Logo -->
